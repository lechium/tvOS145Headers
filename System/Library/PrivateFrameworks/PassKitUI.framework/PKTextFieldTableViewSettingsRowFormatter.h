/*
                       * This header is generated by classdump-dyld 1.5
                       * on Wednesday, April 28, 2021 at 9:08:16 PM Mountain Standard Time
                       * Operating System: Version 14.5 (Build 18L204)
                       * Image Source: /System/Library/PrivateFrameworks/PassKitUI.framework/PassKitUI
                       * classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos. Updated by Kevin Bradley.
                       */


@class NSSet;

@interface PKTextFieldTableViewSettingsRowFormatter : NSObject {

	NSSet* _formatPaddingCharacters;

}

@property (nonatomic,copy) NSSet * formatPaddingCharacters;              //@synthesize formatPaddingCharacters=_formatPaddingCharacters - In the implementation block
-(BOOL)isEqual:(id)arg1 ;
-(unsigned long long)hash;
-(NSSet *)formatPaddingCharacters;
-(id)formattedValueFromInput:(id)arg1 ;
-(id)submissionValueFromFormattedInput:(id)arg1 ;
-(void)setFormatPaddingCharacters:(NSSet *)arg1 ;
@end

