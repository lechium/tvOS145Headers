//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Sep 26 2020 13:48:20).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import "SCATInputSource.h"

@class NSDictionary;
@protocol SCATInterDeviceInputSourceDelegate;

@interface SCATInterDeviceInputSource : SCATInputSource
{
    CDUnknownBlockType _inputDeviceHandledAction;	// 8 = 0x8
    NSDictionary *_actionIdentifiersToRecipeMappingIndices;	// 16 = 0x10
    NSDictionary *_longPressActionIdentifiersToRecipeMappingIndices;	// 24 = 0x18
}

- (void).cxx_destruct;	// IMP=0x00000001000876d0
@property(retain, nonatomic) NSDictionary *longPressActionIdentifiersToRecipeMappingIndices; // @synthesize longPressActionIdentifiersToRecipeMappingIndices=_longPressActionIdentifiersToRecipeMappingIndices;
@property(retain, nonatomic) NSDictionary *actionIdentifiersToRecipeMappingIndices; // @synthesize actionIdentifiersToRecipeMappingIndices=_actionIdentifiersToRecipeMappingIndices;
@property(copy, nonatomic) CDUnknownBlockType inputDeviceHandledAction; // @synthesize inputDeviceHandledAction=_inputDeviceHandledAction;
- (void)_didReceiveActionWithIdentifier:(long long)arg1 start:(_Bool)arg2 ignoreInputHold:(_Bool)arg3;	// IMP=0x0000000100087550
- (void)handleReceivedSwitchEvent:(id)arg1 deviceName:(id)arg2;	// IMP=0x0000000100087118
- (id)_resolveSwitchForSwitchEvent:(id)arg1 deviceName:(id)arg2;	// IMP=0x0000000100086ac8
- (void)_getActionIdentifier:(long long *)arg1 longPressActionIdentifier:(long long *)arg2 forSwitchEvent:(id)arg3;	// IMP=0x0000000100086884
- (void)updateWithSwitches:(id)arg1 recipe:(id)arg2;	// IMP=0x0000000100086590
- (void)setDelegate:(id)arg1 queue:(id)arg2;	// IMP=0x000000010008655c

// Remaining properties
@property(nonatomic) __weak id <SCATInterDeviceInputSourceDelegate> delegate; // @dynamic delegate;

@end

