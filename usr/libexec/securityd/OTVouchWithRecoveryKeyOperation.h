//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Sep 26 2020 13:48:20).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import "CKKSGroupOperation.h"

#import "OctagonStateTransitionOperationProtocol-Protocol.h"

@class NSData, NSOperation, NSString, OTCuttlefishContext, OTOperationDependencies;
@protocol OctagonStateString;

__attribute__((visibility("hidden")))
@interface OTVouchWithRecoveryKeyOperation : CKKSGroupOperation <OctagonStateTransitionOperationProtocol>
{
    _Bool _saveVoucher;	// 10 = 0xa
    NSString<OctagonStateString> *_intendedState;	// 16 = 0x10
    NSString<OctagonStateString> *_nextState;	// 24 = 0x18
    OTCuttlefishContext *_cuttlefishContext;	// 32 = 0x20
    NSData *_voucher;	// 40 = 0x28
    NSData *_voucherSig;	// 48 = 0x30
    OTOperationDependencies *_deps;	// 56 = 0x38
    NSString *_salt;	// 64 = 0x40
    NSString *_recoveryKey;	// 72 = 0x48
    NSOperation *_finishOp;	// 80 = 0x50
}

- (void).cxx_destruct;	// IMP=0x00000001000cea3c
@property(retain) NSOperation *finishOp; // @synthesize finishOp=_finishOp;
@property(retain) NSString *recoveryKey; // @synthesize recoveryKey=_recoveryKey;
@property(retain) NSString *salt; // @synthesize salt=_salt;
@property(retain) OTOperationDependencies *deps; // @synthesize deps=_deps;
@property(readonly) _Bool saveVoucher; // @synthesize saveVoucher=_saveVoucher;
@property(retain, nonatomic) NSData *voucherSig; // @synthesize voucherSig=_voucherSig;
@property(retain, nonatomic) NSData *voucher; // @synthesize voucher=_voucher;
@property __weak OTCuttlefishContext *cuttlefishContext; // @synthesize cuttlefishContext=_cuttlefishContext;
@property(retain) NSString<OctagonStateString> *nextState; // @synthesize nextState=_nextState;
@property(readonly) NSString<OctagonStateString> *intendedState; // @synthesize intendedState=_intendedState;
- (void)proceedWithKeys:(id)arg1 tlkShares:(id)arg2 salt:(id)arg3;	// IMP=0x00000001000ce6e8
- (void)proceedWithRecoveryKeyID:(id)arg1;	// IMP=0x00000001000ce518
- (void)groupStart;	// IMP=0x00000001000cdfa8
- (id)initWithDependencies:(id)arg1 intendedState:(id)arg2 errorState:(id)arg3 recoveryKey:(id)arg4 saveVoucher:(_Bool)arg5;	// IMP=0x00000001000cde7c

@end

